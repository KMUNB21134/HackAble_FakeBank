...
# 8000 (new_api.py, the second automation API) is deliberately not EXPOSEd
# or published anywhere - it's reachable only through app.py's proxy route
# on 5005, same as it is bare-metal. fakebank.db and .rce_flag live inside
# the container's own writable layer, so removing the container
# (docker rm) already wipes them - a fresh `docker run` always starts from
